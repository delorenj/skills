...
- **Hooks and skills fan out from the repo, per-dev.** A repo that adopts the project-scoped agent layer commits one hooks SSOT + one skill set and lets `mise enter/leave` install them into each dev's CLIs (see [references/project-scoped-hooks.md](references/project-scoped-hooks.md)).
