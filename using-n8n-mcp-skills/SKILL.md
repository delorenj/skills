...
1. **Invoke the relevant skill before any n8n action** — not just before MCP calls.
   ...
   you on the highest-impact tool calls _only when the plugin bundle is installed_; on
...
| `n8n-self-hosting`       | _Deployment, not workflow-building_ — self-hosting / installing / deploying n8n on a VM (Docker Compose + Caddy, single vs queue mode), or updating / backing up / hardening it. Triggers on its own; not part of the build flow above.                         |
