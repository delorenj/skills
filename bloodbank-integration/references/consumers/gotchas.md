...
## 5. "FastStream consumer binds but never gets messages" (legacy v2 `/publish`)
...
**Cause.** Producer published with a newer schema; consumer's pinned Bloodbank schema view doesn't know about the field. With `additionalProperties: false`, strict consumers reject it.

**Fix.** Update the consumer's Bloodbank schema checkout or generated local binding to a version that includes the new field. Restart.
