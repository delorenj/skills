...
pulse; hook-hub coalesces revisions with a 2s debounce and a 10s cap, so it runs ~2/s in a busy session,
down from ~9/s; a finished invocation publishes at once) and `bloodbank.system.hook.updated`, and rolls
