.
├── 33god-projects
│   ├── references
│   │   ├── agent-hooks.md
│   │   ├── bmad-init.md
│   │   ├── mise-conventions.md
│   │   └── project-creation.md
│   └── SKILL.md
├── 33god-task-triage
│   └── SKILL.md
├── agent-browser
│   └── SKILL.md
├── agent-team-kit
│   ├── _meta.json
│   ├── agents-config-draft.md
│   ├── package.json
│   ├── README.md
│   ├── SKILL.md
│   └── templates
│       ├── HEARTBEAT.md
│       └── process
│           ├── BACKLOG.md
│           ├── INTAKE.md
│           ├── OPPORTUNITIES.md
│           ├── ROLES.md
│           └── STATUS.md
├── agent-token-efficiency
│   ├── assets
│   │   ├── agents
│   │   │   ├── claude
│   │   │   │   ├── docs-researcher.md
│   │   │   │   ├── log-summarizer.md
│   │   │   │   └── scout.md
│   │   │   ├── codex
│   │   │   │   ├── docs-researcher.toml
│   │   │   │   ├── reviewer.toml
│   │   │   │   └── scout.toml
│   │   │   └── kimi
│   │   │       ├── jarad-scout.yaml
│   │   │       └── prompts
│   │   │           └── jarad-scout.md
│   │   ├── configs
│   │   │   └── providers.yml
│   │   ├── mcp
│   │   │   ├── context7.json
│   │   │   └── semantic-code-search.placeholder.json
│   │   └── templates
│   │       ├── AGENTS.md
│   │       ├── CLAUDE.md
│   │       ├── docs-researcher.prompt.md
│   │       ├── log-summarizer.prompt.md
│   │       └── scout.prompt.md
│   ├── pyproject.toml
│   ├── README.md
│   ├── references
│   │   ├── provider-rotation.md
│   │   ├── supported-agents.md
│   │   └── workflows.md
│   ├── scripts
│   │   ├── __init__.py
│   │   └── agent_efficiency.py
│   ├── SKILL.md
│   └── tests
│       └── test_agent_efficiency.py
├── agno
│   ├── references
│   │   ├── agentos.md
│   │   ├── agents.md
│   │   ├── examples.md
│   │   ├── getting_started.md
│   │   ├── index.md
│   │   ├── integration.md
│   │   ├── llms-full.md
│   │   ├── llms-full.sync-conflict-20260402-113713-DSLLPB2.md
│   │   ├── llms-full.sync-conflict-20260402-113735-DSLLPB2.md
│   │   ├── llms.md
│   │   ├── migration.md
│   │   └── other.md
│   └── SKILL.md
├── ascii-cli-logo-banner
│   ├── examples
│   │   ├── banner-80.md
│   │   └── banner-compact.md
│   ├── LICENSE.txt
│   └── SKILL.md
├── ascii-terminal-animation-pack
│   ├── examples
│   │   ├── poster.md
│   │   └── spec.md
│   ├── LICENSE.txt
│   ├── scripts
│   │   └── matrix_demo.py
│   └── SKILL.md
├── async-pr-review
│   ├── policy.toml
│   ├── scripts
│   │   ├── async-review.sh
│   │   └── check-async-review.sh
│   └── SKILL.md
├── behavioral-evals
│   ├── assets
│   │   ├── interactive_eval.ts.txt
│   │   └── standard_eval.ts.txt
│   ├── references
│   │   ├── creating.md
│   │   ├── fixing.md
│   │   ├── promoting.md
│   │   └── running.md
│   └── SKILL.md
├── bin
│   ├── activate-profile
│   ├── auto-activate
│   └── validate-manifest
├── bloodbank-integration
│   ├── references
│   │   ├── consumers
│   │   │   ├── gotchas.md
│   │   │   ├── methods.md
│   │   │   └── README.md
│   │   ├── hooks
│   │   │   ├── adding-a-harness.md
│   │   │   ├── claude-code.md
│   │   │   ├── github-copilot.md
│   │   │   ├── gotchas.md
│   │   │   └── README.md
│   │   ├── producers
│   │   │   ├── gotchas.md
│   │   │   ├── methods.md
│   │   │   └── README.md
│   │   └── schemas
│   │       ├── gotchas.md
│   │       ├── holyfields.md
│   │       ├── naming.md
│   │       └── README.md
│   └── SKILL.md
├── bloodbank-sdk-generation
│   ├── references
│   │   └── gotchas.md
│   └── SKILL.md
├── bmad-html-workspace
│   ├── references
│   │   └── app-model.md
│   ├── SKILL.md
│   └── templates
│       └── workspace.template.html
├── building-native-ui
│   ├── references
│   │   ├── animations.md
│   │   ├── controls.md
│   │   ├── form-sheet.md
│   │   ├── gradients.md
│   │   ├── icons.md
│   │   ├── media.md
│   │   ├── route-structure.md
│   │   ├── search.md
│   │   ├── storage.md
│   │   ├── tabs.md
│   │   ├── toolbar-and-headers.md
│   │   ├── visual-effects.md
│   │   ├── webgpu-three.md
│   │   └── zoom-transitions.md
│   └── SKILL.md
├── c4-architecture
│   ├── references
│   │   ├── advanced-patterns.md
│   │   ├── advanced-patterns.sync-conflict-20260402-113714-DSLLPB2.md
│   │   ├── advanced-patterns.sync-conflict-20260402-113719-DSLLPB2.md
│   │   ├── c4-syntax.md
│   │   └── common-mistakes.md
│   └── SKILL.md
├── chrome-extension-developer
│   ├── README.md
│   ├── SKILL.md
│   └── WORKFLOW_REFERENCE.md
├── chrome-extension-development
│   └── SKILL.md
├── clerk
│   └── SKILL.md
├── clerk-android
│   ├── references
│   │   ├── custom.md
│   │   └── prebuilt.md
│   └── SKILL.md
├── clerk-backend-api
│   ├── scripts
│   │   ├── api-specs-context.sh
│   │   ├── execute-request.sh
│   │   ├── extract-endpoint-detail.sh
│   │   ├── extract-tag-endpoints.sh
│   │   └── extract-tags.js
│   └── SKILL.md
├── clerk-custom-ui
│   ├── core-2
│   │   ├── custom-sign-in.md
│   │   └── custom-sign-up.md
│   ├── core-3
│   │   ├── custom-sign-in.md
│   │   ├── custom-sign-up.md
│   │   └── show-component.md
│   └── SKILL.md
├── clerk-nextjs-patterns
│   ├── references
│   │   ├── api-routes.md
│   │   ├── caching-auth.md
│   │   ├── middleware-strategies.md
│   │   ├── server-actions.md
│   │   └── server-vs-client.md
│   └── SKILL.md
├── clerk-orgs
│   └── SKILL.md
├── clerk-setup
│   ├── evals
│   │   └── evals.json
│   └── SKILL.md
├── clerk-swift
│   ├── references
│   │   ├── custom.md
│   │   └── prebuilt.md
│   └── SKILL.md
├── clerk-testing
│   └── SKILL.md
├── clerk-webhooks
│   ├── evals
│   │   └── evals.json
│   └── SKILL.md
├── cloudflare
│   ├── references
│   │   ├── agents-sdk
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── ai-gateway
│   │   │   ├── configuration.md
│   │   │   ├── dynamic-routing.md
│   │   │   ├── features.md
│   │   │   ├── README.md
│   │   │   ├── sdk-integration.md
│   │   │   └── troubleshooting.md
│   │   ├── ai-search
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── analytics-engine
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── api
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── api-shield
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── argo-smart-routing
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── artifacts
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   └── README.md
│   │   ├── bindings
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── bot-management
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── browser-rendering
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── c3
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── cache-reserve
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── containers
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── cron-triggers
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── d1
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── ddos
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── do-storage
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   ├── README.md
│   │   │   └── testing.md
│   │   ├── durable-objects
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── email-routing
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── email-workers
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── flagship
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── graphql-api
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── hyperdrive
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── images
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── kv
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── miniflare
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── network-interconnect
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── observability
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── pages
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── pages-functions
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── pipelines
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── pulumi
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── queues
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── r2
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── r2-data-catalog
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── r2-sql
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   ├── README.md
│   │   │   └── SKILL.md.backup
│   │   ├── realtime-sfu
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── realtimekit
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── sandbox
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── secrets-store
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── smart-placement
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── snippets
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── spectrum
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── static-assets
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── stream
│   │   │   ├── api-live.md
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── tail-workers
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── terraform
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── tunnel
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── networking.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── turn
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── turnstile
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── vectorize
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── waf
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── web-analytics
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── integration.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── workerd
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── workers
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── frameworks.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── workers-ai
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── workers-for-platforms
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── workers-playground
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── workers-vpc
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── workflows
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── wrangler
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   └── zaraz
│   │       ├── api.md
│   │       ├── configuration.md
│   │       ├── gotchas.md
│   │       ├── IMPLEMENTATION_SUMMARY.md
│   │       ├── patterns.md
│   │       └── README.md
│   └── SKILL.md
├── cloudflare-agents-sdk
│   ├── references
│   │   ├── browse-the-web.md
│   │   ├── callable.md
│   │   ├── client-sdk.md
│   │   ├── codemode.md
│   │   ├── configuration.md
│   │   ├── durable-execution.md
│   │   ├── email.md
│   │   ├── human-in-the-loop.md
│   │   ├── mcp.md
│   │   ├── observability.md
│   │   ├── queue-retries.md
│   │   ├── routing.md
│   │   ├── server-driven-messages.md
│   │   ├── state-scheduling.md
│   │   ├── streaming-chat.md
│   │   ├── think.md
│   │   ├── voice.md
│   │   ├── webhooks-push.md
│   │   └── workflows.md
│   └── SKILL.md
├── cloudflare-durable-objects
│   ├── references
│   │   ├── rules.md
│   │   ├── testing.md
│   │   └── workers.md
│   └── SKILL.md
├── cloudflare-workers-best-practices
│   ├── references
│   │   ├── review.md
│   │   └── rules.md
│   └── SKILL.md
├── cloudflare-wrangler
│   └── SKILL.md
├── code-reviewer
│   └── SKILL.md
├── coding-strategy
│   └── SKILL.md
├── context-optimizer
│   ├── _meta.json
│   ├── chat-logger.js
│   ├── examples
│   │   ├── clawdbot-integration.js
│   │   └── simple-integration.js
│   ├── INSTALL.md
│   ├── lib
│   │   └── index.js
│   ├── package.json
│   ├── README.md
│   ├── scripts
│   │   └── cli.js
│   ├── SKILL.md
│   └── SUMMARY.md
├── context-self-heal
│   ├── files
│   │   ├── god-13-context-monitor.sh
│   │   ├── god-13-lenoon-monitor.service
│   │   └── god-13-lenoon-monitor.timer
│   └── SKILL.md
├── copilot-money
│   └── SKILL.md
├── creating-coloring-books
│   ├── pyproject.toml
│   ├── scripts
│   │   └── coloring-convert
│   └── SKILL.md
├── delonet-conventions
│   ├── references
│   │   ├── docker_patterns.md
│   │   ├── project_patterns.md
│   │   ├── shell-context-independence.md
│   │   ├── tailscale_audio_forwarding.md
│   │   └── zshyzsh_patterns.md
│   └── SKILL.md
├── delonet-dotenv
│   ├── references
│   │   ├── constants.md
│   │   ├── decisions.md
│   │   ├── gotchas.md
│   │   ├── secrets-vault.md
│   │   └── traefik-routing.md
│   └── SKILL.md
├── delonet-workflow-router
├── design-md
│   ├── examples
│   │   └── DESIGN.md
│   ├── README.md
│   └── SKILL.md
├── digipop-coloring-book-creation
│   └── SKILL.md
├── docs-changelog
│   ├── references
│   │   ├── highlights_examples.md
│   │   ├── index_template.md
│   │   ├── latest_template.md
│   │   └── preview_template.md
│   └── SKILL.md
├── docs-writer
│   ├── quota-limit-style-guide.md
│   └── SKILL.md
├── drumjangler-project-bmad
│   └── SKILL.md
├── elevenlabs-remotion
│   ├── design-system.md
│   ├── dictionaries
│   │   └── example.pls
│   ├── generate.js
│   ├── LICENSE
│   └── SKILL.md
├── elevenlabs-voices
│   ├── examples.md
│   ├── package.json
│   ├── pronunciations.json
│   ├── README.md
│   ├── references
│   │   └── voice-guide.md
│   ├── scripts
│   │   ├── setup.py
│   │   ├── sfx.py
│   │   ├── tts.py
│   │   └── voice-design.py
│   ├── SKILL.md
│   └── voices.json
├── enhance-prompt
│   ├── README.md
│   ├── references
│   │   └── KEYWORDS.md
│   └── SKILL.md
├── event-driven-architecture
│   └── SKILL.md
├── expo
│   ├── references
│   │   ├── api-routes.md
│   │   ├── cicd-workflows.md
│   │   ├── deployment
│   │   │   ├── app-store-metadata.md
│   │   │   ├── ios-app-store.md
│   │   │   ├── play-store.md
│   │   │   ├── testflight.md
│   │   │   └── workflows.md
│   │   ├── deployment.md
│   │   ├── dev-client.md
│   │   ├── ide-integration.md
│   │   ├── module
│   │   │   ├── config-plugin.md
│   │   │   ├── lifecycle.md
│   │   │   ├── module-config.md
│   │   │   ├── native-module.md
│   │   │   └── native-view.md
│   │   ├── module.md
│   │   ├── tailwind-setup.md
│   │   ├── upgrading
│   │   │   ├── expo-av-to-audio.md
│   │   │   ├── expo-av-to-video.md
│   │   │   ├── native-tabs.md
│   │   │   ├── new-architecture.md
│   │   │   ├── react-19.md
│   │   │   └── react-compiler.md
│   │   └── upgrading.md
│   ├── scripts
│   │   ├── fetch.js
│   │   ├── package.json
│   │   └── validate.js
│   └── SKILL.md
├── fal-text-to-image
│   ├── CHEATSHEET.md
│   ├── fal-image-edit
│   ├── fal-image-remix
│   ├── fal-text-to-image
│   │   └── fal-text-to-image
│   ├── pyproject.toml
│   ├── README.md
│   ├── references
│   │   ├── model-comparison.md
│   │   └── usage-examples.md
│   ├── SKILL.md
│   └── uv.lock
├── feedback-vector-loop
│   ├── references
│   │   ├── feedback-data-contract.md
│   │   └── vector-update-policy.md
│   ├── scripts
│   │   ├── compile_feedback_vector.py
│   │   └── rank_candidates_with_vector.py
│   ├── SKILL.md
│   └── workflows
│       └── loop-iteration.md
├── find-skills
│   └── SKILL.md
├── fireflies
│   └── SKILL.md
├── flstudio-scripting
│   ├── references
│   │   ├── api-arrangement-patterns.md
│   │   ├── api-callbacks.md
│   │   ├── api-channels.md
│   │   ├── api-device.md
│   │   ├── api-mixer.md
│   │   ├── api-playlist.md
│   │   ├── api-plugins.md
│   │   ├── api-screen-launchmap.md
│   │   ├── api-transport.md
│   │   ├── api-ui-general.md
│   │   ├── api-utils-constants.md
│   │   ├── examples-patterns.md
│   │   ├── piano-roll-edison.md
│   │   └── pyflp.md
│   ├── SKILL.md
│   └── TONGS
│       ├── COMMIT_EDITMSG
│       ├── config
│       ├── description
│       ├── HEAD
│       ├── hooks
│       │   ├── applypatch-msg.sample
│       │   ├── commit-msg.sample
│       │   ├── fsmonitor-watchman.sample
│       │   ├── post-update.sample
│       │   ├── pre-applypatch.sample
│       │   ├── pre-commit.sample
│       │   ├── pre-merge-commit.sample
│       │   ├── pre-push.sample
│       │   ├── pre-rebase.sample
│       │   ├── pre-receive.sample
│       │   ├── prepare-commit-msg.sample
│       │   ├── push-to-checkout.sample
│       │   ├── sendemail-validate.sample
│       │   └── update.sample
│       ├── index
│       ├── info
│       │   └── exclude
│       ├── logs
│       │   ├── HEAD
│       │   └── refs
│       │       ├── heads
│       │       │   └── main
│       │       └── remotes
│       │           └── origin
│       │               └── main
│       ├── objects
│       │   ├── 09
│       │   │   └── f8210c6d4c1b95fc2a98cc05ed2de9726791b0
│       │   ├── 0c
│       │   │   └── 1bac6bbe76db978fcc1924fdbdd6289740e2c2
│       │   ├── 31
│       │   │   └── 0cd5f89a891c665eb53ec9653da48ab81304fe
│       │   ├── 37
│       │   │   └── 72b05b7481a743b8f24c355be9e6ce0ab6b9ef
│       │   ├── 39
│       │   │   └── ed25e61a64a2967c4e1713899c766a2937f01b
│       │   ├── 42
│       │   │   └── 4630518f62b15849c0c32023f8f25d579aa29b
│       │   ├── 4f
│       │   │   └── 347325329124892a3470dfc682d34784624ccc
│       │   ├── 50
│       │   │   └── 6bd87e63b1d431121c3612de3e4c5b8e92a6d1
│       │   ├── 52
│       │   │   └── b996d6ab9a1facaaf4fc0efdd307721eb1397f
│       │   ├── 70
│       │   │   └── 2d423704733c4227e865f4c140f6d4ca7e5d08
│       │   ├── 7e
│       │   │   └── ee8b1046ad04a1bc2f7201caec668c289183bf
│       │   ├── 96
│       │   │   └── f51030e5782049b6ccf779d574c8d20d8ed6e3
│       │   ├── b8
│       │   │   └── 95eebf05aeac27c24d03ca69e78fdd9496726e
│       │   ├── c1
│       │   │   └── 6b1735e30ca37825eb9d260cc5fd33f7159282
│       │   ├── d3
│       │   │   └── 5ae2388054e42a0931d2be1739b3750a60d144
│       │   ├── ee
│       │   │   └── 56aae56c08781824f9282e95ff5226a80f11ce
│       │   ├── f2
│       │   │   └── 5f65f8c868824d8515f4aa368d3ea399812a97
│       │   └── fe
│       │       └── 519a3e449f9faed39a72dccd9965c48e7ec176
│       └── refs
│           ├── heads
│           │   └── main
│           └── remotes
│               └── origin
│                   └── main
├── follow-up-boss
│   └── SKILL.md
├── github-issue-creator
│   └── SKILL.md
├── god-docs
│   ├── assets
│   │   ├── COMPONENT-GOD-TEMPLATE.md
│   │   ├── DOMAIN-GOD-TEMPLATE.md
│   │   ├── god-check.yml
│   │   └── pre-commit
│   ├── references
│   │   └── spec.md
│   └── SKILL.md
├── google
│   └── SKILL.md
├── hermes-agent-fork-sync
│   └── SKILL.md
├── hermes-pm-template-maintenance
│   └── SKILL.md
├── high-end-visual-design
│   └── SKILL.md
├── hindsight
│   ├── references
│   │   ├── claude-code-journal.md
│   │   ├── cookbook
│   │   │   ├── applications
│   │   │   │   ├── chat-memory.md
│   │   │   │   ├── deliveryman-demo.md
│   │   │   │   ├── hindsight-litellm-demo.md
│   │   │   │   ├── hindsight-tool-learning-demo.md
│   │   │   │   ├── openai-fitness-coach.md
│   │   │   │   ├── sanity-blog-memory.md
│   │   │   │   ├── stancetracker.md
│   │   │   │   └── taste-ai.md
│   │   │   ├── index.md
│   │   │   └── recipes
│   │   │       ├── fitness_tracker.md
│   │   │       ├── healthcare_assistant.md
│   │   │       ├── litellm-memory-demo.md
│   │   │       ├── movie_recommendation.md
│   │   │       ├── per-user-memory.md
│   │   │       ├── personal_assistant.md
│   │   │       ├── personalized_search.md
│   │   │       ├── quickstart.md
│   │   │       ├── study_buddy.md
│   │   │       ├── support-agent-shared-knowledge.md
│   │   │       └── tool-learning-demo.md
│   │   ├── developer
│   │   │   ├── admin-cli.md
│   │   │   ├── api
│   │   │   │   ├── documents.md
│   │   │   │   ├── main-methods.md
│   │   │   │   ├── memory-banks.md
│   │   │   │   ├── mental-models.md
│   │   │   │   ├── operations.md
│   │   │   │   ├── quickstart.md
│   │   │   │   ├── recall.md
│   │   │   │   ├── reflect.md
│   │   │   │   └── retain.md
│   │   │   ├── configuration.md
│   │   │   ├── development.md
│   │   │   ├── extensions.md
│   │   │   ├── index.md
│   │   │   ├── installation.md
│   │   │   ├── mcp-server.md
│   │   │   ├── models.md
│   │   │   ├── monitoring.md
│   │   │   ├── multilingual.md
│   │   │   ├── observations.md
│   │   │   ├── performance.md
│   │   │   ├── rag-vs-hindsight.md
│   │   │   ├── reflect.md
│   │   │   ├── retain.md
│   │   │   ├── retrieval.md
│   │   │   ├── retrieval.sync-conflict-20260402-113713-DSLLPB2.md
│   │   │   ├── retrieval.sync-conflict-20260402-113719-DSLLPB2.md
│   │   │   ├── services.md
│   │   │   └── storage.md
│   │   └── sdks
│   │       ├── cli.md
│   │       ├── embed.md
│   │       ├── integrations
│   │       │   ├── ai-sdk.md
│   │       │   ├── litellm.md
│   │       │   ├── local-mcp.md
│   │       │   ├── openclaw.md
│   │       │   └── skills.md
│   │       ├── nodejs.md
│   │       └── python.md
│   ├── scripts
│   │   ├── audit_hindsight_memory.py
│   │   └── hs-bank-id.sh
│   └── SKILL.md
├── html-project-console
│   ├── commands
│   │   └── bootstrap-project-console.md
│   ├── references
│   │   └── design-principles.md
│   ├── SKILL.md
│   └── templates
│       └── project-console.html
├── hyperframes
│   ├── data-in-motion.md
│   ├── house-style.md
│   ├── palettes
│   │   ├── bold-energetic.md
│   │   ├── clean-corporate.md
│   │   ├── dark-premium.md
│   │   ├── jewel-rich.md
│   │   ├── monochrome.md
│   │   ├── nature-earth.md
│   │   ├── neon-electric.md
│   │   ├── pastel-soft.md
│   │   └── warm-editorial.md
│   ├── patterns.md
│   ├── references
│   │   ├── audio-reactive.md
│   │   ├── beat-direction.md
│   │   ├── captions.md
│   │   ├── css-patterns.md
│   │   ├── design-picker.md
│   │   ├── dynamic-techniques.md
│   │   ├── motion-principles.md
│   │   ├── narration.md
│   │   ├── prompt-expansion.md
│   │   ├── techniques.md
│   │   ├── transcript-guide.md
│   │   ├── transitions
│   │   │   ├── catalog.md
│   │   │   ├── css-3d.md
│   │   │   ├── css-blur.md
│   │   │   ├── css-cover.md
│   │   │   ├── css-destruction.md
│   │   │   ├── css-dissolve.md
│   │   │   ├── css-distortion.md
│   │   │   ├── css-grid.md
│   │   │   ├── css-light.md
│   │   │   ├── css-mechanical.md
│   │   │   ├── css-other.md
│   │   │   ├── css-push.md
│   │   │   ├── css-radial.md
│   │   │   └── css-scale.md
│   │   ├── transitions.md
│   │   ├── tts.md
│   │   ├── typography.md
│   │   └── video-composition.md
│   ├── scripts
│   │   ├── animation-map.mjs
│   │   ├── contrast-report.mjs
│   │   └── package-loader.mjs
│   ├── SKILL.md
│   ├── templates
│   │   └── design-picker.html
│   └── visual-styles.md
├── installing-apps-tools-and-services
│   ├── assets
│   │   ├── docker-compose.template.yml
│   │   └── traefik-labels.yml
│   ├── references
│   │   ├── docker-compose-patterns.md
│   │   ├── native-service-connection.md
│   │   └── traefik-labels-reference.md
│   ├── scripts
│   │   ├── install-node-cli.sh
│   │   └── install-python-cli.sh
│   └── SKILL.md
├── just-fucking-cancel
│   ├── assets
│   │   └── template.html
│   ├── PUBLISH.md
│   ├── references
│   │   └── common-services.md
│   └── SKILL.md
├── karpathy-llm-wiki
│   ├── assets
│   │   └── karpathy-tweet.png
│   ├── examples
│   │   ├── 2026-03-19-claude-code-statusline-landscape.md
│   │   ├── ai-coding-tools-index.md
│   │   ├── claude-code-statusline-landscape.md
│   │   ├── log-sample.md
│   │   └── README.md
│   ├── LICENSE
│   ├── README.md
│   ├── references
│   │   ├── archive-template.md
│   │   ├── article-template.md
│   │   ├── index-template.md
│   │   └── raw-template.md
│   └── SKILL.md
├── kimi-webbridge
│   ├── references
│   │   └── operations.md
│   ├── scripts
│   │   └── screenshot.sh
│   └── SKILL.md
├── launch-strategy
│   ├── evals
│   │   └── evals.json
│   └── SKILL.md
├── managing-tickets-and-tasks-in-plane
│   ├── assets
│   │   ├── changelog-email.md
│   │   └── ticket-template.md
│   ├── hooks
│   │   ├── lib
│   │   │   └── workspace-detector.sh
│   │   ├── pre-prompt.sh
│   │   └── stop.sh
│   ├── references
│   │   ├── audit-board-state.md
│   │   ├── create-ticket-from-story.md
│   │   ├── plane-json-template.json
│   │   ├── project-initialization.md
│   │   └── select-next-ticket.md
│   └── SKILL.md
├── mise-task-managing
│   ├── references
│   │   ├── base-mise.toml
│   │   ├── brownfield.md
│   │   ├── common-workflows.md
│   │   ├── starting-fresh.md
│   │   ├── workflows.md
│   │   └── wrapping-an-interface.md
│   └── SKILL.md
├── mise-tasks
│   ├── references
│   │   ├── advanced.md
│   │   ├── arguments.md
│   │   ├── bootstrap-monorepo.md
│   │   ├── env-integration.md
│   │   ├── evolution-log.md
│   │   ├── patterns.md
│   │   ├── polyglot-affected.md
│   │   ├── release-workflow-patterns.md
│   │   └── task-levels.md
│   └── SKILL.md
├── mise-versioning
│   ├── assets
│   │   ├── version-tasks.toml
│   │   └── versioning.sh
│   ├── mise-versioning.skill
│   ├── references
│   │   ├── file-types.md
│   │   └── gotchas.md
│   ├── scripts
│   │   └── init.sh
│   └── SKILL.md
├── mise.toml
├── monitoring-stack
│   ├── references
│   │   ├── alert-rules-guide.md
│   │   └── architecture.md
│   ├── scripts
│   │   └── stack-health.sh
│   ├── SKILL.md
│   ├── SKILL.sync-conflict-20260402-113714-DSLLPB2.md
│   └── SKILL.sync-conflict-20260402-113716-DSLLPB2.md
├── n8n
│   ├── references
│   │   ├── code-javascript
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── code-python
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── expressions
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── mcp-tools
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── node-configuration
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   ├── validation
│   │   │   ├── api.md
│   │   │   ├── configuration.md
│   │   │   ├── gotchas.md
│   │   │   ├── patterns.md
│   │   │   └── README.md
│   │   └── workflow-patterns
│   │       ├── ai-agent-workflow.md
│   │       ├── api.md
│   │       ├── configuration.md
│   │       ├── database-operations.md
│   │       ├── gotchas.md
│   │       ├── http-api-integration.md
│   │       ├── patterns.md
│   │       ├── README.md
│   │       ├── scheduled-tasks.md
│   │       └── webhook-processing.md
│   └── SKILL.md
├── native-data-fetching
│   ├── references
│   │   └── expo-router-loaders.md
│   └── SKILL.md
├── notebooklm
│   ├── images
│   │   └── example_notebookchat.png
│   ├── LICENSE
│   ├── notebooklm
│   ├── README.md
│   ├── references
│   │   ├── api_reference.md
│   │   ├── troubleshooting.md
│   │   └── usage_patterns.md
│   ├── requirements.txt
│   ├── scripts
│   │   ├── __init__.py
│   │   ├── ask_question.py
│   │   ├── auth_manager.py
│   │   ├── browser_session.py
│   │   ├── cleanup_manager.py
│   │   ├── notebook_manager.py
│   │   ├── run.py
│   │   └── setup_environment.py
│   └── SKILL.md
├── oauth-helper
│   └── SKILL.md
├── obsidian-vault-management
│   └── SKILL.md
├── openclaw-runtime-git
│   └── SKILL.md
├── openclaw-upgrade
│   ├── QUICKREF.md
│   ├── scripts
│   │   └── upgrade.sh
│   └── SKILL.md
├── opencode-controller
│   ├── assets
│   │   └── operator-prompts.md
│   ├── references
│   │   ├── command-cheatsheet.md
│   │   ├── failure-handling.md
│   │   ├── model-selection.md
│   │   ├── plan-vs-build.md
│   │   ├── question-handling.md
│   │   ├── session-management.md
│   │   └── workflow.md
│   └── SKILL.md
├── openrouter-transcribe
│   ├── scripts
│   │   └── transcribe.sh
│   └── SKILL.md
├── phone-agent
│   ├── README.md
│   ├── scripts
│   │   ├── requirements.txt
│   │   ├── server_realtime.py
│   │   └── server.py
│   ├── SKILL.md
│   └── tasks
│       ├── book_restaurant.yaml
│       └── get_quote.yaml
├── pikastream-video-meeting
│   ├── assets
│   │   └── placeholder-avatar.jpg
│   ├── requirements.txt
│   ├── scripts
│   │   └── pikastreaming_videomeeting.py
│   └── SKILL.md
├── pirate-skill
│   └── SKILL.md
├── pixijs
│   ├── references
│   │   ├── accessibility.md
│   │   ├── application
│   │   │   ├── application-options.md
│   │   │   └── index.md
│   │   ├── assets
│   │   │   ├── background.md
│   │   │   ├── bundles.md
│   │   │   ├── caching.md
│   │   │   ├── compressed-textures.md
│   │   │   ├── fonts.md
│   │   │   ├── gif.md
│   │   │   ├── index.md
│   │   │   ├── manifests.md
│   │   │   ├── progress.md
│   │   │   ├── resolution.md
│   │   │   ├── spritesheet.md
│   │   │   ├── svg.md
│   │   │   └── video.md
│   │   ├── blend-modes.md
│   │   ├── color.md
│   │   ├── core-concepts
│   │   │   ├── index.md
│   │   │   ├── render-loop.md
│   │   │   └── renderers.md
│   │   ├── create.md
│   │   ├── custom-rendering
│   │   │   ├── index.md
│   │   │   └── uniform-types.md
│   │   ├── environments.md
│   │   ├── events.md
│   │   ├── filters.md
│   │   ├── index.md
│   │   ├── math.md
│   │   ├── migration-v8.md
│   │   ├── performance.md
│   │   ├── scene-container.md
│   │   ├── scene-core-concepts
│   │   │   ├── constructor-options.md
│   │   │   ├── container-hierarchy.md
│   │   │   ├── index.md
│   │   │   ├── layers.md
│   │   │   ├── masking.md
│   │   │   ├── render-groups.md
│   │   │   ├── scene-management.md
│   │   │   └── transforms.md
│   │   ├── scene-dom-container.md
│   │   ├── scene-gif.md
│   │   ├── scene-graphics.md
│   │   ├── scene-mesh
│   │   │   ├── index.md
│   │   │   ├── mesh-perspective.md
│   │   │   ├── mesh-plane.md
│   │   │   ├── mesh-rope.md
│   │   │   ├── mesh-simple.md
│   │   │   └── mesh.md
│   │   ├── scene-particle-container.md
│   │   ├── scene-sprite
│   │   │   ├── animated-sprite.md
│   │   │   ├── index.md
│   │   │   ├── nineslice-sprite.md
│   │   │   ├── sprite.md
│   │   │   └── tiling-sprite.md
│   │   ├── scene-text
│   │   │   ├── bitmap-text.md
│   │   │   ├── html-text.md
│   │   │   ├── index.md
│   │   │   ├── split-bitmap-text.md
│   │   │   ├── split-text.md
│   │   │   └── text.md
│   │   └── ticker.md
│   └── SKILL.md
├── pjangler-dev
│   ├── references
│   │   ├── command-interface.md
│   │   └── recipe-interface.md
│   └── SKILL.md
├── pm2
│   └── SKILL.md
├── pr-address-comments
│   ├── scripts
│   │   └── fetch-pr-info.js
│   └── SKILL.md
├── pr-creator
│   └── SKILL.md
├── pricing-strategy
│   ├── evals
│   │   └── evals.json
│   ├── references
│   │   ├── research-methods.md
│   │   └── tier-structure.md
│   └── SKILL.md
├── product-changelog
│   └── SKILL.md
├── project-crank-engine
│   ├── references
│   │   ├── prompt-templates.md
│   │   ├── prompt-templates.sync-conflict-20260402-113715-DSLLPB2.md
│   │   └── prompt-templates.sync-conflict-20260402-113721-DSLLPB2.md
│   ├── scripts
│   │   └── render_crank_prompt.py
│   └── SKILL.md
├── rabbitmq-management
├── ralph-wiggum-v2
│   └── SKILL.md
├── react-components
│   ├── examples
│   │   └── gold-standard-card.tsx
│   ├── package-lock.json
│   ├── package.json
│   ├── README.md
│   ├── resources
│   │   ├── architecture-checklist.md
│   │   ├── component-template.tsx
│   │   ├── stitch-api-reference.md
│   │   └── style-guide.json
│   ├── scripts
│   │   ├── fetch-stitch.sh
│   │   └── validate.js
│   └── SKILL.md
├── remotion
│   ├── examples
│   │   ├── screens.json
│   │   └── WalkthroughComposition.tsx
│   ├── README.md
│   ├── resources
│   │   ├── composition-checklist.md
│   │   └── screen-slide-template.tsx
│   ├── scripts
│   │   └── download-stitch-asset.sh
│   └── SKILL.md
├── review-duplication
│   └── SKILL.md
├── security-monitor
│   ├── scripts
│   │   └── monitor.cjs
│   └── SKILL.md
├── self-improving-agent
│   ├── hooks
│   │   ├── post-bash.sh
│   │   ├── pre-tool.sh
│   │   └── session-end.sh
│   ├── memory
│   │   └── semantic-patterns.json
│   ├── README.md
│   ├── references
│   │   └── appendix.md
│   ├── runtime
│   │   ├── __init__.py
│   │   └── learning_core.py
│   ├── scripts
│   │   └── hook_runtime.py
│   ├── SKILL.md
│   ├── templates
│   │   ├── correction-template.md
│   │   ├── pattern-template.md
│   │   └── validation-template.md
│   └── tests
│       └── test_hook_runtime.py
├── shadcn-components
│   └── SKILL.md
├── shadcn-ui
│   ├── examples
│   │   ├── auth-layout.tsx
│   │   ├── data-table.tsx
│   │   └── form-pattern.tsx
│   ├── README.md
│   ├── resources
│   │   ├── component-catalog.md
│   │   ├── customization-guide.md
│   │   ├── migration-guide.md
│   │   └── setup-guide.md
│   ├── scripts
│   │   └── verify-setup.sh
│   └── SKILL.md
├── skill-creator
│   ├── LICENSE.txt
│   ├── mise-task-managing.skill
│   ├── references
│   │   ├── anatomy.md
│   │   ├── design-principles.md
│   │   ├── frontmatter-guide.md
│   │   ├── gotchas.md
│   │   ├── migration.md
│   │   ├── output-patterns.md
│   │   ├── routing-mechanics.md
│   │   ├── taxonomy-templates.md
│   │   ├── topology-decision.md
│   │   └── workflows.md
│   ├── scripts
│   │   ├── init_skill.py
│   │   ├── package_skill.py
│   │   └── quick_validate.py
│   └── SKILL.md
├── skill-tree.md
├── skills.manifest.yaml
├── stacks-deploy
│   ├── references
│   │   ├── deployment-modes.md
│   │   ├── secret-injection.md
│   │   ├── stacks-layout.md
│   │   └── traefik-labels.md
│   └── SKILL.md
├── starship-customization
│   ├── references
│   │   ├── gotchas.md
│   │   ├── recipes.md
│   │   └── syntax.md
│   ├── SKILL.md
│   └── starship-customization.skill
├── stealth-browser
│   ├── scripts
│   │   ├── cf_bypass.py
│   │   ├── login_session.py
│   │   ├── proxy_rotate.py
│   │   ├── session_manager.py
│   │   ├── smart_login.py
│   │   ├── solve_captcha.py
│   │   ├── stealth_session.py
│   │   └── task_runner.py
│   └── SKILL.md
├── stitch-design
│   ├── examples
│   │   ├── DESIGN.md
│   │   └── enhanced-prompt.md
│   ├── README.md
│   ├── references
│   │   ├── design-mappings.md
│   │   ├── prompt-keywords.md
│   │   └── tool-schemas.md
│   ├── SKILL.md
│   └── workflows
│       ├── edit-design.md
│       ├── generate-design-md.md
│       └── text-to-design.md
├── stitch-loop
│   ├── examples
│   │   ├── next-prompt.md
│   │   └── SITE.md
│   ├── README.md
│   ├── resources
│   │   ├── baton-schema.md
│   │   └── site-template.md
│   ├── SKILL.md
│   └── stitch-design -> ../../all-skills/stitch-design
├── string-reviewer
│   ├── references
│   │   ├── settings.md
│   │   └── word-list.md
│   └── SKILL.md
├── subagent-driven-development
│   ├── code-quality-reviewer-prompt.md
│   ├── implementer-prompt.md
│   ├── SKILL.md
│   └── spec-reviewer-prompt.md
├── tailnet-multi-machine
│   └── SKILL.md
├── tanstack-start-best-practices
│   ├── rules
│   │   ├── api-routes.md
│   │   ├── auth-route-protection.md
│   │   ├── auth-session-management.md
│   │   ├── deploy-adapters.md
│   │   ├── env-functions.md
│   │   ├── err-server-errors.md
│   │   ├── file-separation.md
│   │   ├── mw-request-middleware.md
│   │   ├── sf-create-server-fn.md
│   │   ├── sf-input-validation.md
│   │   ├── ssr-hydration-safety.md
│   │   ├── ssr-prerender.md
│   │   └── ssr-streaming.md
│   └── SKILL.md
├── taste-design
│   ├── resources
│   │   └── DESIGN.md
│   └── SKILL.md
├── tauri
│   ├── references
│   │   ├── core_concepts.md
│   │   ├── development.md
│   │   ├── distribution.md
│   │   ├── getting_started.md
│   │   ├── index.md
│   │   ├── llms-full.md
│   │   ├── llms.md
│   │   ├── other.md
│   │   ├── plugins.md
│   │   ├── reference.md
│   │   ├── security.md
│   │   └── tutorials.md
│   └── SKILL.md
├── temporal-python-testing
│   ├── resources
│   │   ├── integration-testing.md
│   │   ├── local-setup.md
│   │   ├── replay-testing.md
│   │   └── unit-testing.md
│   └── SKILL.md
├── tflo-agent-orchestrator
│   └── SKILL.md
├── tflo-create-pr
│   └── SKILL.md
├── tflo-implement
│   └── SKILL.md
├── tflo-setup
│   └── SKILL.md
├── tflo-triage
│   └── SKILL.md
├── tflo-verify
│   └── SKILL.md
├── ui-ux-design
│   ├── _meta.json
│   ├── references
│   │   ├── ACCESSIBILITY.md
│   │   ├── COMPONENTS.md
│   │   └── DESIGN_SYSTEM.md
│   ├── SKILL.md
│   └── UI_UX_MASTER_GUIDE.md
├── unfuck-my-git-state
│   ├── agents
│   │   └── openai.yaml
│   ├── references
│   │   ├── recovery-checklist.md
│   │   └── symptom-map.md
│   ├── scripts
│   │   ├── guided_repair_plan.sh
│   │   ├── regression_harness.sh
│   │   └── snapshot_git_state.sh
│   └── SKILL.md
├── use-dom
│   └── SKILL.md
├── vercel-composition-patterns
│   ├── AGENTS.md
│   ├── metadata.json
│   ├── README.md
│   ├── rules
│   │   ├── _sections.md
│   │   ├── _template.md
│   │   ├── architecture-avoid-boolean-props.md
│   │   ├── architecture-compound-components.md
│   │   ├── patterns-children-over-render-props.md
│   │   ├── patterns-explicit-variants.md
│   │   ├── react19-no-forwardref.md
│   │   ├── state-context-interface.md
│   │   ├── state-decouple-implementation.md
│   │   └── state-lift-state.md
│   └── SKILL.md
├── vercel-react-best-practices
│   ├── AGENTS.md
│   ├── metadata.json
│   ├── README.md
│   ├── rules
│   │   ├── _sections.md
│   │   ├── _template.md
│   │   ├── advanced-event-handler-refs.md
│   │   ├── advanced-init-once.md
│   │   ├── advanced-use-latest.md
│   │   ├── async-api-routes.md
│   │   ├── async-defer-await.md
│   │   ├── async-dependencies.md
│   │   ├── async-parallel.md
│   │   ├── async-suspense-boundaries.md
│   │   ├── bundle-barrel-imports.md
│   │   ├── bundle-conditional.md
│   │   ├── bundle-defer-third-party.md
│   │   ├── bundle-dynamic-imports.md
│   │   ├── bundle-preload.md
│   │   ├── client-event-listeners.md
│   │   ├── client-localstorage-schema.md
│   │   ├── client-passive-event-listeners.md
│   │   ├── client-swr-dedup.md
│   │   ├── js-batch-dom-css.md
│   │   ├── js-cache-function-results.md
│   │   ├── js-cache-property-access.md
│   │   ├── js-cache-storage.md
│   │   ├── js-combine-iterations.md
│   │   ├── js-early-exit.md
│   │   ├── js-hoist-regexp.md
│   │   ├── js-index-maps.md
│   │   ├── js-length-check-first.md
│   │   ├── js-min-max-loop.md
│   │   ├── js-min-max-loop.sync-conflict-20260402-113713-DSLLPB2.md
│   │   ├── js-min-max-loop.sync-conflict-20260402-113714-DSLLPB2.md
│   │   ├── js-set-map-lookups.md
│   │   ├── js-tosorted-immutable.md
│   │   ├── rendering-activity.md
│   │   ├── rendering-animate-svg-wrapper.md
│   │   ├── rendering-conditional-render.md
│   │   ├── rendering-content-visibility.md
│   │   ├── rendering-hoist-jsx.md
│   │   ├── rendering-hydration-no-flicker.md
│   │   ├── rendering-hydration-suppress-warning.md
│   │   ├── rendering-svg-precision.md
│   │   ├── rendering-usetransition-loading.md
│   │   ├── rerender-defer-reads.md
│   │   ├── rerender-dependencies.md
│   │   ├── rerender-derived-state-no-effect.md
│   │   ├── rerender-derived-state.md
│   │   ├── rerender-functional-setstate.md
│   │   ├── rerender-lazy-state-init.md
│   │   ├── rerender-memo-with-default-value.md
│   │   ├── rerender-memo.md
│   │   ├── rerender-move-effect-to-event.md
│   │   ├── rerender-simple-expression-in-memo.md
│   │   ├── rerender-transitions.md
│   │   ├── rerender-use-ref-transient-values.md
│   │   ├── server-after-nonblocking.md
│   │   ├── server-auth-actions.md
│   │   ├── server-cache-lru.md
│   │   ├── server-cache-react.md
│   │   ├── server-dedup-props.md
│   │   ├── server-parallel-fetching.md
│   │   └── server-serialization.md
│   └── SKILL.md
├── vercel-react-native-skills
│   ├── AGENTS.md
│   ├── metadata.json
│   ├── README.md
│   ├── rules
│   │   ├── _sections.md
│   │   ├── _template.md
│   │   ├── animation-derived-value.md
│   │   ├── animation-gesture-detector-press.md
│   │   ├── animation-gpu-properties.md
│   │   ├── design-system-compound-components.md
│   │   ├── fonts-config-plugin.md
│   │   ├── imports-design-system-folder.md
│   │   ├── js-hoist-intl.md
│   │   ├── list-performance-callbacks.md
│   │   ├── list-performance-function-references.md
│   │   ├── list-performance-images.md
│   │   ├── list-performance-inline-objects.md
│   │   ├── list-performance-item-expensive.md
│   │   ├── list-performance-item-memo.md
│   │   ├── list-performance-item-types.md
│   │   ├── list-performance-virtualize.md
│   │   ├── monorepo-native-deps-in-app.md
│   │   ├── monorepo-single-dependency-versions.md
│   │   ├── navigation-native-navigators.md
│   │   ├── react-compiler-destructure-functions.md
│   │   ├── react-compiler-reanimated-shared-values.md
│   │   ├── react-state-dispatcher.md
│   │   ├── react-state-fallback.md
│   │   ├── react-state-minimize.md
│   │   ├── rendering-no-falsy-and.md
│   │   ├── rendering-text-in-text-component.md
│   │   ├── scroll-position-no-state.md
│   │   ├── state-ground-truth.md
│   │   ├── ui-expo-image.md
│   │   ├── ui-image-gallery.md
│   │   ├── ui-measure-views.md
│   │   ├── ui-menus.md
│   │   ├── ui-native-modals.md
│   │   ├── ui-pressable.md
│   │   ├── ui-safe-area-scroll.md
│   │   ├── ui-scrollview-content-inset.md
│   │   └── ui-styling.md
│   ├── SKILL.md
│   ├── vercel-composition-patterns -> ../../all-skills/vercel-composition-patterns
│   └── vercel-react-best-practices -> ../../all-skills/vercel-react-best-practices
├── vox-tts
│   ├── references
│   │   ├── integrations.md
│   │   ├── troubleshooting.md
│   │   └── voice_design.md
│   ├── scripts
│   │   ├── add_voice.sh
│   │   ├── register_mcp.sh
│   │   ├── synth_url.sh
│   │   └── synth.sh
│   └── SKILL.md
├── web-design-guidelines
│   └── SKILL.md
└── zellij-plugin-dev
    ├── references
    │   ├── index.md
    │   ├── other.md
    │   ├── plugin-api-commands.md
    │   ├── plugin-api-events.md
    │   ├── plugin-development-tutorial.md
    │   ├── plugin-examples-external-integration.md
    │   ├── plugin-examples-status-theming.md
    │   └── plugin-examples-ui-navigation.md
    └── SKILL.md

419 directories, 1340 files
