...
| SVG                 | `.svg`                                                           | `svg`                         | `loadSvg` (see `svg.md`)                                                                                             |
| Video textures      | `.mp4`, `.m4v`, `.webm`, `.ogg`, `.ogv`, `.h264`, `.avi`, `.mov` | `video`                       | `loadVideoTextures` (see `video.md`)                                                                                 |
| Sprite sheets       | `.json` (Spritesheet format)                                     | `spritesheet`                 | `spritesheetAsset` (see `spritesheet.md`)                                                                            |
| Bitmap fonts        | `.fnt`, `.xml`                                                   | `bitmap-font`                 | `loadBitmapFont` (loading works by default; rendering `BitmapText` requires `'pixi.js/text-bitmap'`; see `fonts.md`) |
| Web fonts           | `.ttf`, `.otf`, `.woff`, `.woff2`                                | `web-font`                    | `loadWebFont` (see `fonts.md`)                                                                                       |
| JSON                | `.json`                                                          | `json`                        | `loadJson`                                                                                                                      |
| Text                | `.txt`                                                           | `text`                        | `loadTxt`                                                                                                                       |
| Compressed textures | `.basis`, `.dds`, `.ktx`, `.ktx2`                                | `basis`, `dds`, `ktx`, `ktx2` | See `compressed-textures.md`                                                                                         |
| Animated GIFs       | `.gif`                                                           | `gif`                         | Requires `'pixi.js/gif'`; returns `GifSource` (see `gif.md`)                                                         |
...
| ------------------------------ | ---------------------------------------------------------------------- | ------------------------------------------- |
| Texture atlases and animations | [spritesheet.md](spritesheet.md)                 | Loading sprite sheets with `AnimatedSprite` |
| Video textures                 | [video.md](video.md)                             | `.mp4`, `.webm`, autoplay, looping, mobile  |
| Web and bitmap fonts           | [fonts.md](fonts.md)                             | `.woff2`, `.fnt`, font families, SDF fonts  |
| Animated GIFs                  | [gif.md](gif.md)                                 | `.gif`, `GifSprite`, playback control       |
| Grouping assets by feature     | [bundles.md](bundles.md)                         | `addBundle`, `loadBundle`, `unloadBundle`   |
| Declaring everything upfront   | [manifests.md](manifests.md)                     | `Assets.init({ manifest })` workflows       |
| Cache lookups and cleanup      | [caching.md](caching.md)                         | `Assets.get`, `Assets.unload`, `Cache`      |
| Priming future assets          | [background.md](background.md)                   | `backgroundLoad`, `backgroundLoadBundle`    |
| Loading screens                | [progress.md](progress.md)                       | `onProgress`, LoadOptions progress          |
| GPU-compressed formats         | [compressed-textures.md](compressed-textures.md) | `.ktx2`, `.basis`, `.dds`, `.ktx`           |
| Vector vs raster SVG           | [svg.md](svg.md)                                 | `parseAsGraphicsContext`, texture mode      |
| Retina + format detection      | [resolution.md](resolution.md)                   | `@{1,2}x`, `format` preferences             |

## Decision guide

- **Need to load a single image?** Use `Assets.load(url)`. No setup required.
- **Loading many assets grouped by level/scene?** Use a bundle. See `bundles.md`.
- **Know all assets at build time?** Use a manifest in `Assets.init`. See `manifests.md`.
- **Need a loading bar?** Pass a progress callback to `Assets.load`. See `progress.md`.
- **Smooth transitions between levels?** Background-load the next level. See `background.md`.
- **Memory budget matters?** Use compressed textures and `Assets.unload` between screens. See `compressed-textures.md` and `caching.md`.
- **Need crisp SVG icons at any size?** Load as Graphics, not texture. See `svg.md`.
- **Retina + WebP/AVIF?** Configure `texturePreference` and use format patterns. See `resolution.md`.
...
| ------------------ | ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| Texture (image)    | `TextureSourceOptions`                                                          | `resolution`, `scaleMode`, `alphaMode`, `autoGenerateMipmaps`, `antialias`, `addressMode`                                                                                                                                                                  | `resolution.md`          |
| SVG                | `{ parseAsGraphicsContext?, resolution? }`                                      | `parseAsGraphicsContext` for Graphics mode; `resolution` for sharper raster                                                                                                                                                                                | `svg.md`                 |
| Video              | `VideoSourceOptions`                                                            | `autoPlay`, `loop`, `muted`, `playsinline`, `preload`, `updateFPS`, `crossorigin`, `mime`                                                                                                                                                                  | `video.md`               |
| Web font           | `LoadFontData`                                                                  | `family`, `weights`, `style`, `display`, `unicodeRange`, `featureSettings`                                                                                                                                                                                 | `fonts.md`               |
| Bitmap font        | (none; auto-configured)                                                         | Distance-field detection sets scale mode and mipmaps                                                                                                                                                                                                       | `fonts.md`               |
| Spritesheet        | `{ texture?, imageFilename?, ignoreMultiPack?, textureOptions?, cachePrefix? }` | `textureOptions` forwards `TextureSourceOptions` (e.g. `scaleMode`) to the atlas image; `texture` to skip image load; `imageFilename` to override the referenced image; `ignoreMultiPack` to skip multi-pack follow-ups; `cachePrefix` to namespace frames | `spritesheet.md`         |
| GIF                | `GifBufferOptions`                                                              | `fps`, `scaleMode`, `resolution`, `autoGenerateMipmaps`                                                                                                                                                                                                    | `gif.md`                 |
| Compressed texture | `TextureSourceOptions`                                                          | `scaleMode`, `addressMode`, `autoGenerateMipmaps`                                                                                                                                                                                                          | `compressed-textures.md` |
