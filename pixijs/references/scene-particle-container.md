...
**Related references:** `scene-core-concepts` (scene graph basics), `scene-sprite` (when you need full features per object), `assets` (shared textures, atlases), `performance` (batching, texture optimization), `scene-container` (wrap with other display objects).

## Constructor options

### ParticleContainerOptions

All `Container` options (`position`, `scale`, `tint`, `label`, `filters`, `zIndex`, etc.) are also valid here — see `scene-core-concepts/constructor-options.md`. Note that `children` is omitted: use `particles` instead.
