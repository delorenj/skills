...
Assumes familiarity with `scene-core-concepts`.
...
**Related references:** `scene-core-concepts` (scene graph mental model, masking, layers, render groups), `scene-sprite` / `scene-graphics` / `scene-text` / `scene-mesh` (leaf objects that go inside containers), `events` (`eventMode`, hit testing), `math` (Matrix, toGlobal/toLocal detail), `performance` (`cacheAsTexture`, culling, render groups).
...
All `Container` options (`position`, `scale`, `tint`, `label`, `filters`, `zIndex`, etc.) are also valid here — see `scene-core-concepts/constructor-options.md`.
...
`cullable` and `cullArea` are valid constructor options (the `assignWithIgnore` pass copies them like any other field), but they are documented in `performance` because culling setup is a performance concern rather than a scene-graph concern.
...
For render-order control that is decoupled from the hierarchy (children keep their logical parent for transforms but render at a different z), use `RenderLayer`. See `scene-core-concepts/scene-management.md`.
...
These are emitted on the `EventEmitter` side of `Container`; do not confuse them with pointer events from `events`.
