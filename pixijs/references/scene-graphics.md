...
Assumes familiarity with `scene-core-concepts`. `Graphics` is a leaf: do not nest children inside it. Wrap multiple `Graphics` objects in a `Container` to group them.
...
**Related references:** `scene-core-concepts` (scene graph basics), `scene-container` (group graphics with other objects), `scene-core-concepts/masking.md` (Graphics as a stencil mask), `filters` (effects), `performance` (batching, `cacheAsTexture`).
