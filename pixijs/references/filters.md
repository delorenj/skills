...
**Related references:** `custom-rendering` (shader internals, uniform types), `blend-modes` (composing with filters), `performance` (filter tuning, filterArea).
...
Advanced blend modes (`color-burn`, `overlay`, `hard-light`, etc.) are powered by the filter system and must be imported before use. They also require `useBackBuffer: true` on WebGL; see the `blend-modes` skill for the full list.
