...
This skill is the shared mental model referenced by all `scene-*` leaves. It explains what the scene graph is in PixiJS v8, how a `Container` differs from a leaf, and where each concept lives. It does not go deep on any single API; it frames the pieces and points to the skill or reference file that does.
...
**Related references:** `scene-container` (Container API in detail), the leaf skills (`scene-sprite`, `scene-graphics`, `scene-text`, `scene-mesh`, `scene-particle-container`, `scene-dom-container`, `scene-gif`), `events` (hit testing traverses the scene graph), `performance` (cache, culling, render groups), `math` (Matrix, toGlobal/toLocal detail).
...
Containers emit events for hierarchy and visibility changes: `childAdded` / `childRemoved` on the parent, `added` / `removed` on the child, plus `visibleChanged` and `destroyed` on the container itself. Useful for wiring reactive UI updates or resource bookkeeping. Full details in `container-hierarchy.md`.

## Leaf comparison: which skill covers which object

| Leaf                                                                 | Primary use                                                                                                                                                                 | Skill                             |
| -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| `Sprite`                                                             | Draw a single texture at a position (with variants `NineSliceSprite` for resizable UI panels and `TilingSprite` for repeating backgrounds).                                 | `scene-sprite`             |
| `Text` / `BitmapText` / `HTMLText` / `SplitText` / `SplitBitmapText` | Render text. Canvas-based `Text` for general use, `BitmapText` for high-volume cheap text, `HTMLText` for rich HTML/CSS layout, split variants for per-character animation. | `scene-text`               |
| `Graphics`                                                           | Vector drawing: shapes, lines, paths, fills, strokes. Backed by a `GraphicsContext`.                                                                                        | `scene-graphics`           |
| `Mesh` / `MeshSimple` / `MeshPlane` / `MeshRope` / `PerspectiveMesh` | Custom geometry with a shader or texture. Use `MeshRope` for textured path-following ribbons and `PerspectiveMesh` for 2D perspective.                                      | `scene-mesh`               |
| `ParticleContainer` + `Particle`                                     | Thousands of lightweight sprites with a restricted transform set, for high-throughput particle effects.                                                                     | `scene-particle-container` |
| `DOMContainer`                                                       | Render an HTML element positioned inside the scene graph (useful for inputs, iframes, accessibility overlays).                                                              | `scene-dom-container`      |
| `GifSprite`                                                          | Animated GIF playback as a display object. Requires `pixi.js/gif`.                                                                                                          | `scene-gif`                |

`Container` itself is covered in `scene-container` and is the node every leaf lives inside.

## When to use what (quick decisions)

- "I want to group and transform some display objects" → `Container`, see `scene-container`.
- "I want to draw a texture" → `Sprite`, see `scene-sprite`.
- "I want to draw vector shapes or paths" → `Graphics`, see `scene-graphics`.
- "I want to draw text" → `Text` / `BitmapText` / `HTMLText`, see `scene-text`.
- "I want thousands of cheap sprites" → `ParticleContainer`, see `scene-particle-container`.
- "I want a custom-geometry mesh or a deformed sprite" → `Mesh` or one of its variants, see `scene-mesh`.
- "I want to clip a sub-tree" → set `.mask`, see `masking.md`.
- "I want a decoupled render order" → `RenderLayer`, see `scene-management.md`.
- "I want GPU-level transforms for a big stable sub-tree" → `isRenderGroup: true`, see `scene-management.md`.
- "I want to skip offscreen rendering" → `cullable = true` + `CullerPlugin`, see `performance`.

## References

- [constructor-options.md](constructor-options.md): the ~30 fields inherited by every `Container`-derived node (transform, display, hierarchy, sorting, layout, effects, callbacks), with defaults, types, and when line-by-line assignment is appropriate. Shared reference for all leaf skills.
- [container-hierarchy.md](container-hierarchy.md): add/remove/swap children, reparenting with transform preservation, label navigation, destroy sub-trees.
- [transforms.md](transforms.md): position, scale, rotation, pivot, origin, skew, toGlobal/toLocal, the three matrices (local/group/world), bounds.
- [masking.md](masking.md): AlphaMask, StencilMask, ScissorMask, ColorMask, inverse masking, cost comparison.
- [layers.md](layers.md): `RenderLayer`, attach/detach, sorted layers, layer + logical parent split.
- [render-groups.md](render-groups.md): `isRenderGroup`, GPU-level transforms, when to use, render-groups vs `cacheAsTexture`.
- [scene-management.md](scene-management.md): combined view; render groups, `RenderLayer`, culling, zIndex sorting, `boundsArea`.
