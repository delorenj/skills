...
**Related references:** `core-concepts` (renderers, render pipeline), `ticker` (render loop detail), `scene-container` (working with `app.stage`), `environments` (non-browser setups).
...
For every option — view/canvas, background, renderer preference (including the array form), ticker, resize, culler, events, accessibility, WebGL/WebGPU context flags, Graphics bezier smoothness, GC, and per-renderer overrides (`webgl` / `webgpu` / `canvasOptions`) — see [application-options.md](application-options.md).
...
`app.stage` is a plain `Container`. For scene graph detail (transforms, addChild, destroy) see `scene-container`. For renderer-level operations (extract, generateTexture, custom systems) see `core-concepts` and `custom-rendering`. `app.domContainerRoot` is the `<div>` that the renderer uses to host `DOMContainer` overlays; append it next to `app.canvas` when you need DOM elements pinned to scene nodes (see `scene-dom-container`).
...
The callback receives the `Ticker` instance; read `ticker.deltaTime` for a frame-rate-independent multiplier (~1.0 at 60fps), `ticker.deltaMS` for real milliseconds, or `ticker.FPS` for the current frame rate. See `ticker` for priorities, FPS capping, `onRender`, shared vs private tickers, and the v8 callback signature change.
