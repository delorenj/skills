...
Assumes familiarity with `scene-core-concepts`. `DOMContainer` extends `ViewContainer`, so it's a leaf: do not nest PixiJS children inside it. Nest DOM content inside the HTML element itself, or wrap multiple `DOMContainer` instances in a `Container`. Not available in Web Workers; a worker has no DOM to overlay.
...
**Related references:** `scene-core-concepts` (scene graph basics), `scene-container` (wrap multiple DOM overlays), `events` (pointer handling on canvas vs DOM), `accessibility` (screen-reader overlays).
