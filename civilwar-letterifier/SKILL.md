...
On any non-normal streamed-body exit, the adapter aborts its request controller
synchronously, then best-effort starts cancellation of the unread reader/body
without awaiting it before classifying the error. Late cleanup failures never
replace that error or authorize a retry. Fully consumed streams are not
cancelled.
